...
> протягом усієї побудови, не тільки після неї.
...
**Дії:**
1. 🙋 Людина: установити [Ollama](https://ollama.com) і запустити застосунок; установити [Obsidian](https://obsidian.md).
2. Запусти `bash scripts/setup.sh`. Перший прогін створить `scripts/config.toml` — відредагуй шляхи під цю машину (єдине місце шляхів, див. коментарі в файлі). Запусти setup ще раз: він створить venv, стягне bge-m3 (≈1.1 ГБ, разово), створить теки і symlinks.

**✅ Перевірка:** `scripts/setup.sh` завершується рядком «Готово»; `curl -s localhost:11434/api/tags | grep bge-m3` щось знаходить.
...
Embed локальний (Ollama bge-m3, $0, приватно). Індекс інкрементний по sha — наступні прогони копійчані. Деталі, ваги джерел і банери недовіри — [docs/04-semantics.md](docs/04-semantics.md).
