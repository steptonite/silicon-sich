...
> **V2:** для нового користувача спочатку виконай `python3 sbkit.py doctor`,
> `.venv/bin/python sbkit.py demo`, а потім `install --profile guided`.
> Нижче лишається повний advanced-маршрут і пояснення кожного шару.
...
- Підтверджені дії → `task.py`; не змішувати з inbox або memory ([docs/11](docs/11-tasks.md)).
- Локальні агентні сесії → ручний `brain_refresh.py`; автозапуск лише після явної згоди ([docs/12](docs/12-live-refresh.md)).
- Нові куровані ноти → `distill_delta.py` + library-check перед індексацією ([docs/13](docs/13-library-maintenance.md)).
